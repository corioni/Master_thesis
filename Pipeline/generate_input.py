...
dictfile       = "parameters_Sim_30.json"
...
class Generate:
  ...
  def __init__(self, run_param):
    ...
    self.inputdir = '/uio/hume/student-u24/martacor/Simulation/FML/FML/COLASolver/input_Sim/mu0_trial'
    self.outputdir = '/uio/hume/student-u24/martacor/Simulation/FML/FML/COLASolver/output_Sim/mu0_trial'

    # Folder to store everything (use the full path here)
    self.inputfolder = self.inputdir# run_param['outputfolder']+'/'+self.outputdir
    self.outputfolder = self.outputdir#run_param['outputfolder']+'/'+self.inputdir
    if not (os.path.isdir(self.outputfolder)):
      subprocess.run(["mkdir", self.outputfolder])
    if not(os.path.isdir(self.inputfolder)):
      subprocess.run(["mkdir", self.inputfolder])
  ...
  def generate_data(self, GR):
    ...
    params = {
        'root': self.inputfolder+'/class_'+self.name+'_',
   ...
   # [3] Extract and output transfer information (in CAMB format suitable for COLA)
    colatransferinfofile = self.inputdir + ' '  + str(len(zarr))
#    print(zarr)
    for _z in zarr:
      # Transfer
      ...
      np.savetxt(self.inputfolder + "/" + pofkfilename, np.c_[karr, pofk_cb, pofk_total], header="# k (h/Mpc)  P_tot(k)   P_cb(k)  (Mpc/h)^3")
      ...
      np.savetxt(self.inputfolder + "/" + filename, np.c_[output.T], 
    ...
    colatransferinfofilename = self.inputdir + "/class_transferinfo_"+self.name+".txt"
    colatransferinfofiledir = self.inputfolder + "/class_transferinfo_"+self.name+".txt"

    with open(colatransferinfofiledir, 'w') as f:
...
if gravity_model == \"Marta\" then                           \n\
 -- Parameter mu0, for GR = 0.0                              \n\
  gravity_model_marta_mu0 = "+str(self.gravity_model_marta_mu0)+"                                \n\
end                                                          \n\
                                                             \n\
------------------------------------------------------------ \n\
-- Particles                                                 \n\
