...
template <int NDIM, class T>
void output_density_slices(NBodySimulation<NDIM, T> & sim, double redshift, std::string snapshot_folder, int nmesh) {
    std::stringstream stream;
    stream << std::fixed << std::setprecision(3) << redshift;
    std::string redshiftstring = stream.str();
    ...
    if (FML::ThisTask == 0) {
        std::cout << "\n";
        std::cout << "#=====================================================\n";
        std::cout << "# Computing density slices\n";
    ...
    FFTWGrid<NDIM> density_grid(nmesh, nleftright.first, nleftright.second);
    ...
    // Output
    //=============================================================
    auto Local_nx = density_grid.get_local_nx();
    auto Local_x_start = density_grid.get_local_x_start();
    for (int islice = 0; islice < Local_nx; islice++) {
        // The number of the slice
        int islice_global = islice + Local_x_start;
        // Open file for output
        std::string filename = snapshot_folder + "/density_slice_z" + redshiftstring + "." + std::to_string(islice_global);
        std::ofstream fp(filename.c_str());

        for (auto && real_index : density_grid.get_real_range(islice, islice + 1)) {
            // The index (i,j,k) of the cell
            auto coord = density_grid.get_coord_from_index(real_index);
            // The value delta(i,j,k)
            auto delta = density_grid.get_real_from_index(real_index);
        fp << coord[1] << " " << coord[2] << " " << delta << "\n";
        }
    }
}
